...
MNEM_TAB mnem_rules[] = {
        {"unpr",     DOLLAR_UNPR},
        // rule fails if a prefix has been removed
        {"noprefix", DOLLAR_NOPREFIX},
        // a pronunciation is given in the *_list file
        {"list",     DOLLAR_LIST},
        {"w_alt1", 0x11},
        {"w_alt2", 0x12},
        {"w_alt3", 0x13},
        ...
        {"w_alt5", 0x15},
        {"w_alt6", 0x16},
        // note: put longer names before their sub-strings
        {"w_alt",  0x11},
...
MNEM_TAB mnem_flags[] = {
        ...
        {"$6",           0x46},
        {"$7",           0x47},
        // reduce to unstressed
        {"$u",           0x48},
        {"$u1",          0x49},
        {"$u2",          0x4a},
        {"$u3",          0x4b},
        // reduce to unstressed, but stress at end of clause
        {"$u+",          0x4c},
        {"$u1+",         0x4d},
        {"$u2+",         0x4e},
        {"$u3+",         0x4f},
        ...
        // ensure pause before this word
        {"$pause",       8},
        // full stress if at end of clause
        {"$strend",      9},
        // full stress if at end of clause, or only followed by unstressed
        {"$strend2",     10},
        // reduce stress at end of clause
        {"$unstressend", 11},
        // use this pronuciation rather than split into letters
        {"$abbrev",      13},

        // language specific
        // IT double the initial consonant of next word
        {"$double",      14},
        // use alternative pronunciation
        {"$alt",         15},
        // synonym for $alt
        {"$alt1",        15},
        {"$alt2",        16},
        ...
        {"$combine",     23},
        // ignore '.' after this word (abbreviation)
        {"$dot",         24},
        // use this pronunciation if there is a dot after the word
        {"$hasdot",      25},
        // limit to 3 repetitions
        {"$max3",        27},
        // a shorter $pause
        {"$brk",         28},
        // word translates to replcement text, not phonemes
        {"$text",        29},

        // flags in dictionary word 2
        // verb follows
        {"$verbf",       0x20},
        // verb follows, allow -s suffix
        {"$verbsf",      0x21},
        // noun follows
        {"$nounf",       0x22},
        // past tense follows
        {"$pastf",       0x23},
        // use this pronunciation when its a verb
        {"$verb",        0x24},
        // use this pronunciation when its a noun
        {"$noun",        0x25},
        // use this pronunciation when its past tense
        {"$past",        0x26},
        // extend influence of 'verb follows'
        {"$verbextend",  0x28},
        // use this pronunciation if initial letter is upper case
        {"$capital",     0x29},
        // use this pronunciation if initial letter is upper case
        {"$allcaps",     0x2a},
        // character name is base-character name + accent name
        {"$accent",      0x2b},
        // only if this clause is a sentence (i.e. terminator is {. ? !} not {, ; :}
        {"$sentence",    0x2d},
        // only match on this word without suffix
        {"$only",        0x2e},
        // only match with none, or with 's' suffix
        {"$onlys",       0x2f},
        // must have a suffix
        {"$stem",        0x30},
        // use this pronunciation if at end of clause
        {"$atend",       0x31},
        // use this pronunciation at start of clause
        {"$atstart",     0x32},
        // not if we've switched translators
        {"$native",      0x33},

        // doesn't set dictionary_flags
        // conditional rule, followed by byte giving the condition number
        {"$?",           100},
...
const char *LookupMnemName(
        MNEM_TAB *table, const int value) {
...
void print_dictionary_flags(
    ...
    if ((stress = flags[0] & 0xf) != 0) {
        sprintf(buf, "%s", LookupMnemName(
                mnem_flags, stress + 0x40));
        total = (int) strlen(buf);
        buf += total;
    }

    for (ix = 8; ix < 64; ix++) {
        if (((ix < 30) && (flags[0] & (1 << ix))) ||
            ((ix >= 0x20) && (flags[1] & (1 << (ix - 0x20))))) {
...
static int compile_line(
    ...
    int len_word;
    int len_phonetic;
    // this word specifies replacement text, not phonemes
    int text_not_phonemes;
    unsigned int wc;
    int all_upper_case;
    ...
    while (c != '\n') {
        ...
        if ((c == '?') && (step == 0)) {
            // conditional rule, allow only if
            // the numbered condition is set for the voice
            flag_offset = 100;
    ...
    if (text_not_phonemes) {
        if (word[0] == '_') {
            // This is a special word, used by eSpeak.
            // Translate this into phonemes now
            // need a space to indicate word-boundary
            strcat(phonetic, " ");

            // PROBLEM  vowel reductions are
            // not applied to the translated phonemes
        ...
        if (strchr(encoded_ph, phonSWITCH) != 0) {
            // don't match on suffixes (except 's') when switching languages
        ...
        if (bad_phoneme != 0) {
            ...
            fprintf(f_log, "%5d: Bad phoneme [%s] (U+%x) in: %s  %s\n",
                    espeak_linenum, bad_phoneme_str, bad_phoneme,
                    word, phonetic);
            error_count++;
        }
    }
    ...
    if (sscanf(word, "U+%x", &wc) == 1) {
        ...
        // note if the word is all-capitals
        int c2;

        all_upper_case = 1;
        p = word;
        for (p = word;;) {
            // this assumes that the lower case char
            // is the same length as the upper case char
            // OK, except for Turkish "I", but
            // use towlower() rather than towlower2()
            ix = utf8_in(&c2, p);
            if (c2 == 0)
                break;
            if (iswupper2(c2)) {
    ...
    *hash = HashDictionary(word);
    len_phonetic = (int) strlen(encoded_ph);

    // bit 6 indicates whether the word has been compressed
...
static void compile_dictlist_start() {
    int ix;
    char *p;
    char *p2;
...
static int compile_dictlist_file(
...
static void copy_rule_string(
        char *string, int *state_out) {
    static char *outbuf[5] =
            {rule_cond, rule_pre, rule_match,
    ...
    output = outbuf[state];
    if (state == 4) {
        // append to any previous phoneme string,
        // i.e. allow spaces in the phoneme string
        len = strlen(rule_phonemes);
        if (len > 0)
            rule_phonemes[len++] = ' ';
        output = &rule_phonemes[len];
    }
    // to ensure non-zero bytes
    sxflags = 0x808000;
...
//endif
        ...
        if ((state == 1) || (state == 3)) {
            ...
            // (note: 'E' is reserved for a replaced silent 'e')
            if (literal == 0) {
                static const char lettergp_letters[9] =
                        {LETTERGP_A, LETTERGP_B, LETTERGP_C, 0, 0, LETTERGP_F,
                         LETTERGP_G, LETTERGP_H, LETTERGP_Y};
                switch (c) {
                    case '_':
                        c = RULE_SPACE;
                        break;
                    ...
                    case 'L':
                        ...
                        c += 'A';
                        if (state == 1) {
                            // pre-rule, put the group number
                            // before the RULE_LETTERGP command
                            output[ix++] = c;
                            c = RULE_LETTERGP2;
                        } else {
...
static char *compile_rule(char *input) {
    ...
    if (bad_phoneme != 0) {
        ...
        fprintf(f_log, "%5d: Bad phoneme [%s] (U+%x) in: %s\n",
                espeak_linenum, bad_phoneme_str, bad_phoneme, input);
        error_count++;
    }
    strcpy(output, buf);
    len = strlen(buf) + 1;
    ...
    if ((len_name > 0) && (memcmp(rule_match, group_name, len_name) != 0)) {
        ...
        if ((group_name[0] == '9') && IsDigit(wc)) {
            ...
            fprintf(f_log, "%5d: Wrong initial letters '%s' for group '%s'\n",
                    espeak_linenum, rule_match, group_name);
            error_count++;
        }
    }
    ...
    if (debug_flag) {
        ...
        output[len + 1] = (espeak_linenum % 255) + 1;
        output[len + 2] = (espeak_linenum / 255) + 1;
        len += 3;
    }
    ...
    if (rule_pre[0] != 0) {
        ...
        for (ix = (int) strlen(rule_pre) - 1; ix >= start; ix--)
            output[len++] = rule_pre[ix];
    }

    if (rule_post[0] != 0) {
        sprintf(&output[len], "%c%s", RULE_POST, rule_post);
        len += (int) (strlen(rule_post) + 1);
    }
    output[len++] = 0;
...
static int __cdecl rgroup_sorter(RGROUP *a, RGROUP *b) {
    int ix;
    ix = (int) (strlen(b->name) - strlen(a->name));
    if (ix != 0) return (ix);
...
static void print_rule_group(
        FILE *f_out, int n_rules,
        char **rules, char *name) {
    int rule;
    int ix;
    unsigned char c;
    int len1;
    ...
    for (rule = 0; rule < n_rules; rule++) {
        ...
        *pout = 0;
        ...
        if (condition > 0) {
            ...
            spaces -= (int) strlen(buf);
            fprintf(f_out, "%s", buf);
        }

        if (rule_pre[0] != 0) {
            p = buf;
            for (ix = (int) strlen(rule_pre) - 1; ix >= 0; ix--)
                *p++ = rule_pre[ix];
            sprintf(p, ") ");
            spaces -= (int) strlen(buf);
...
static void output_rule_group(
    ...
    common = "";
    qsort((void *) rules, n_rules, sizeof(char *),
          (int (__cdecl *)(const void *, const void *)) string_sorter);

    //  don't remove characters from numeric match strings
    if (strcmp(name, "9") == 0)
        len_name = 0;
    ...
    for (ix = 0; ix < n_rules; ix++) {
        ...
        len1 = (int) strlen(p) + 1;
        p3 = &p[len1];
        // remove group name from start of match string
        p2 = p3 + len_name;
        len2 = (int) strlen(p2);

        // the next byte after the group name
        nextchar_count[(unsigned char) (p2[0])]++;

        if ((common[0] != 0) && (strcmp(p, common) == 0)) {
            fwrite(p2, len2, 1, f_out);
            // no phoneme string, it's the same as previous rule
            fputc(0, f_out);
        } else {
            if ((ix < n_rules - 1) && (strcmp(p, rules[ix + 1]) == 0)) {
                // phoneme string is same as next, set as common
                common = rules[ix];
...
static int compile_lettergroup(
        char *input, FILE *f_out) {
    char *p;
    char *p_start;
    int group;
    int ix;
...
#define N_LETTERGP_ITEMS 200
    ...
    if (group >= N_LETTER_GROUPS) {
        fprintf(f_log, "%5d: lettergroup out of range (01-%.2d)\n",
                espeak_linenum, N_LETTER_GROUPS - 1);
        error_count++;
        return (1);
    }
...
static int compile_dictrules(
    ...
    for (;;) {
        ...
        if ((buf == NULL) || (buf[0] == '.')) {
            ...
            if (compile_mode == 2) {
                ...
                // write a zero word to terminate the replacemenmt list
            ...
            if (memcmp(buf, ".group", 6) == 0) {
                ...
                // Note: Windows isspace(0xe1) gives TRUE !
                while ((p[0] == ' ') || (p[0] == '\t')) p++;
                ix = 0;
                ...
                if ((group3_ix == 0) && (strlen(group_name) > 2)) {
                    if (utf8_in(&c, group_name) < 2) {
                        fprintf(f_log, "%5d: Group name longer than 2 bytes (UTF8)", espeak_linenum);
                        error_count++;
                    }
        ...
        switch (compile_mode) {
            ...
            case 2: //  .replace
            {
                ...
                if (replace1 != 0) {
                    ...
                    Write4Bytes(f_out, replace1);
                    // if big-endian, reverse the bytes in LoadDictionary()
                    Write4Bytes(f_out, replace2);
                }
            }
                break;
        }
    ...
    qsort((void *) rgroup, n_rgroups, sizeof(rgroup[0]),
          (int (__cdecl *)(const void *, const void *)) rgroup_sorter);
...
int CompileDictionary(
        const char *dsource, const char *dict_name,
        FILE *log, char *fname_err, int flags) {
    FILE *f_in;
    FILE *f_out;
    int offset_rules = 0;
    int value;
