...
extern void print_dictionary_flags(
        unsigned int *flags,
        char *buf, int buf_len);

extern char *DecodeRule(
        const char *group_chars,
        int group_length,
        char *rule, int control);
...
static const unsigned short diereses_list[7] =
        {0xe4, 0xeb, 0xef, 0xf6, 0xfc, 0xff, 0};
...
static void InitGroups(Translator *tr) {
    ...
    while (*p != 0) {
        ...
        if (p[0] == RULE_REPLACEMENTS) {
            ...
            pw = (unsigned int *) (((long64) p + 4) & ~3);
            tr->langopts.replace_chars = pw;
            while (pw[0] != 0) {
                // find the end of the replacement list, each entry is 2 words.
                pw += 2;
            }
            p = (char *) (pw + 1);
...
int LoadDictionary(Translator *tr, const char *name,
                   int no_error) {
    int hash;
    char *p;
    int *pw;
    ...
    // currently loaded dictionary name
    strcpy(dictionary_name, name);
    ...
    if ((tr->dict_min_size > 0) &&
        (size < (unsigned int) tr->dict_min_size)) {
...
*/
const char *EncodePhonemes(
        const char *p, char *outptr,
        int *bad_phoneme) {
    int ix;
    unsigned char c;
    ...
    while (((c = *p) != 0) && !isspace(c)) {
        ...
        switch (c) {
            case '|':
                ...
                // to prevent characters being treated
            ...
            default:
                // lookup the phoneme mnemonic,
                // find the phoneme with the highest number of
                // matching characters
                max = -1;
                max_ph = 0;
                ...
                for (ix = 1; ix < n_phoneme_tab; ix++) {
                    ...
                    if (phoneme_tab[ix]->type == phINVALID)
                        // this phoneme is not defined for this language
                        continue;
...
// passes the result of GetTranslatedPhonemeString()
static char *phon_out_buf = NULL;
static int phon_out_size = 0;


char *WritePhMnemonic(
        char *phon_out, PHONEME_TAB *ph,
        PHONEME_LIST *plist, int use_ipa,
        int *flags) {
    int c;
    int mnem;
    int len;
...
const char *GetTranslatedPhonemeString(int phoneme_mode) {
    ...
    static const char *stress_chars = "==,,''";
    // combining-double-inverted-breve, zero-width-joiner
    static const int char_tie[] = {0x0361, 0x200d};
    ...
    if (phon_out_buf == NULL) {
        ...
        if ((phon_out_buf = (char *) realloc(
                phon_out_buf, phon_out_size)) == NULL) {
            phon_out_size = 0;
            return ("");
        }
    }
        ...
        if (plist->newword)
            *buf++ = ' ';
            ...
            if ((separate_phonemes != 0) && (ix > 1)) {
                utf8_in(&c, phon_buf2);
                // not if the phoneme starts with a superscript letter
                if ((c < 0x2b0) || (c > 0x36f)) {
...
//                        *buf++ = separate_phonemes;
        ...
        for (p = phon_buf2; *p != 0;) {
            p += utf8_in(&c, p);
            if (use_tie > 0) {
                // look for non-inital alphabetic character,
                // but not diacritic, superscript etc.
                if ((count > 0) && !(flags & (1 << (count - 1))) &&
                    ((c < 0x2b0) || (c > 0x36f)) && iswalpha2(c)) {
        ...
        if (plist->ph->code != phonSWITCH) {
            ...
            if ((plist->synthflags & SFLAG_SYLLABLE) &&
                ...
                // syllablic consonant
                buf = WritePhMnemonic(
                        buf, phoneme_tab[phonSYLLABIC],
                        NULL, use_ipa, NULL);
            }
            if (plist->tone_ph > 0) {
                buf = WritePhMnemonic(
                        buf, phoneme_tab[plist->tone_ph],
                        NULL, use_ipa, NULL);
            }
        }
...
static int IsLetterGroup(
        Translator *tr, char *word,
        int group, int pre) {
    char *p;
    char *w;
    int len = 0;
...
static int Unpronouncable2(
    ...
    end_flags = TranslateRules(
            tr, word, ph_buf, sizeof(ph_buf),
            NULL, FLAG_UNPRON_TEST, NULL);
    word[-1] = c;
    if ((end_flags == 0) ||
        (end_flags & SUFX_UNPRON))
        return (1);
    return (0);
}
...
int Unpronouncable(
    ...
    if ((tr->letter_bits_offset > 0) && (c < 0x241)) {
        ...
        // so we can re-translate the word as English
        return (0);
    }
    ...
    if (c1 == tr->langopts.param[LOPT_UNPRONOUNCABLE])
        // disregard this as the initial letter when counting
        vowel_posn--;

    if (vowel_posn > (tr->langopts.max_initial_consonants + 1))
        // no vowel, or no vowel in first few letters
        return (1);
...
static int GetVowelStress(
        Translator *tr, unsigned char *phonemes,
        signed char *vowel_stress, int *vowel_count,
        int *stressed_syllable, int control) {
    unsigned char phcode;
    PHONEME_TAB *ph;
...
void ChangeWordStress(Translator *tr, char *word,
                      int new_stress) {
    int ix;
    unsigned char *p;
    int max_stress;
...
*/
void SetWordStress(
        Translator *tr, char *output,
        unsigned int *dictionary_flags,
        int tonic, int control) {
    unsigned char phcode;
    unsigned char *p;
    PHONEME_TAB *ph;
    int stress;
    ...
    static char consonant_types[16] =
    ...
    max_stress = GetVowelStress(
            tr, phonetic, vowel_stress,
            &vowel_count, &stressed_syllable, 1);
    ...
    for (p = phonetic; *p != 0; p++) {
        if ((phoneme_tab[p[0]]->type == phVOWEL) &&
            ...
            if (consonant_types[phoneme_tab[p[1]]->type] &&
                ...
                // followed by two consonants, a long consonant,
                // or consonant and end-of-word
                weight++;
            }
    ...
    switch (tr->langopts.stress_rule) {
        case 8:
            // stress on first syllable, unless it is a
            // light syllable followed by a heavy syllable
            ...
            if (stressed_syllable == 0) {
                ...
                if (vowel_count > 2) {
                    ...
                    if (stressflags & (S_FINAL_SPANISH | S_FINAL_STRESS_C)) {
                        // LANG=Spanish, stress on last vowel if the word
                        // ends in a consonant other than 'n' or 's'
                        if (phoneme_tab[final_ph]->type != phVOWEL) {
                            if (stressflags & S_FINAL_STRESS_C) {
                                ...
                                if (tr->translator_name == L('a', 'n')) {
                                    ...
                                    } else if (((phoneme_tab[final_ph]->type != phNASAL) && (mnem != 's')) ||
                                               (phoneme_tab[final_ph2]->type != phVOWEL)) {
                                        stressed_syllable = vowel_count - 1;
                                    }
                                }
                    ...
                    if (stressflags & S_FINAL_LONG) {
                        // stress on last syllable if it has a long vowel,
                        // but previous syllable has a short vowel
                        if (vowel_length[vowel_count - 1] >
                            vowel_length[vowel_count - 2]) {
                            stressed_syllable = vowel_count - 1;
                        }
                    }
        ...
        case 5:
            ...
            if (stressed_syllable == 0) {
                ...
                static char guess_ru[16] =
                        {0, 0, 1, 1, 2, 3, 3, 4, 5, 6, 7, 7, 8, 9, 10, 11};
                // for final phoneme is a vowel
                static char guess_ru_v[16] =
                        {0, 0, 1, 1, 2, 2, 3, 3, 4, 5, 6, 7, 7, 8, 9, 10};
                // for final phoneme is an unvoiced stop
                static char guess_ru_t[16] =
                        {0, 0, 1, 2, 3, 3, 3, 4, 5, 6, 7, 7, 7, 8, 9, 10};
        ...
        case 6:    // LANG=hi stress on the last heaviest syllable
            if (stressed_syllable == 0) {
                ...
                if ((syllable_weight[vowel_count - 1] == 2) &&
                    (max_weight < 2)) {
                    // the only double=heavy syllable
                    // is the final syllable, so stress this
                    stressed_syllable = vowel_count - 1;
                } else if (max_weight <= 0) {
                    // all syllables, exclusing the last,
                    // are light. Stress the first syllable
                    stressed_syllable = 1;
                }
        ...
        case 7:
            // LANG=tr, the last syllable for
            // any vowel marked explicitly as unstressed
            if (stressed_syllable == 0) {
                stressed_syllable = vowel_count - 1;
                for (ix = 1; ix < vowel_count; ix++) {
        ...
        case 12:  // LANG=kl (Greenlandic)
            ...
            for (ix = 1; ix < vowel_count; ix++) {
                if (vowel_stress[ix] == 4)
                    // change marked stress (consonant clusters)
                    // to secondary (except the last)
                    vowel_stress[ix] = 3;
        ...
        case 13:
            // LANG=ml, 1st unless 1st vowel is short and 2nd is long
            if (stressed_syllable == 0) {
                stressed_syllable = 1;
                if ((vowel_length[1] == 0) &&
                    (vowel_count > 2) &&
    ...
    if (unstressed_word == 0) {
        if ((stressflags & S_2_SYL_2) && (vowel_count == 3)) {
            // Two syllable word, if one syllable has primary stress,
            // then give the other secondary stress
            if (vowel_stress[1] == 4)
                vowel_stress[2] = 3;
            if (vowel_stress[2] == 4)
                vowel_stress[1] = 3;
        ...
        if ((stressflags & S_INITIAL_2) && (vowel_stress[1] < 0)) {
            // If there is only one syllable before
            // the primary stress, give it a secondary stress
        ...
        if (vowel_stress[v] < 0) {
            if ((stressflags & S_FINAL_NO_2) &&
                ...
                if ((v > 1) && (stressflags & S_2_TO_HEAVY) &&
                    (syllable_weight[v] == 0) &&
                    (syllable_weight[v + 1] > 0)) {
                    // don't put secondary stress on a light syllable
                    // which is followed by a heavy syllable
                    continue;
                }
...
// the primary stress and put secondary stress on alternate syllables?
    ...
    if ((unstressed_word) && (tonic < 0)) {
        if (vowel_count <= 2)
            /* monosyllable - unstressed */
            tonic = tr->langopts.unstressed_wd1;
        else
            /* more than one syllable, used secondary stress as the main stress */
            tonic = tr->langopts.unstressed_wd2;
    }
    ...
    while (((phcode = *p++) != 0) && (output < max_output)) {
        ...
        if (ph->type == phPAUSE) {
        ...
        } else if (((ph->type == phVOWEL) &&
                    !(ph->phflags & phNONSYLLABIC)) ||
                   (*p == phonSYLLABIC)) {
            ...
            if (v_stress <= 1) {
                if ((v > 1) && (max_stress >= 2) &&
                    (stressflags & S_FINAL_DIM) &&
                    (v == (vowel_count - 1))) {
                    // option: mark unstressed final syllable as diminished
                    v_stress = 0;
                } else if ((stressflags & S_NO_DIM) ||
                           (v == 1) || (v == (vowel_count - 1))) {
                    // first or last syllable, or
                    // option 'don't set diminished stress'
                    v_stress = 1;
                } else if ((v == (vowel_count - 2)) &&
                           (vowel_stress[vowel_count - 1] <= 1)) {
                    // penultimate syllable, followed
                    // by an unstressed final syllable
                    v_stress = 1;
                } else {
            ...
            if ((*p == phonLENGTHEN) &&
                ((opt_length = tr->langopts.param[LOPT_IT_LENGTHEN]) & 1)) {
                // remove lengthen indicator from non-stressed syllables
                int shorten = 0;

                if (opt_length & 0x10) {
                    // only allow lengthen indicator on
                    // the highest stress syllable in the word
            ...
            if ((v_stress >= 4) &&
                ...
                // LANG=Italian, lengthen penultimate stressed vowels,
                // unless followed by 2 consonants
                if ((v == (vowel_count - 2)) &&
                    (syllable_weight[v] == 0)) {
                    *output++ = phcode;
                    phcode = phonLENGTHEN;
...
*/
void AppendPhonemes(
        Translator *tr, char *string,
        int size, const char *ph) {
    const char *p;
    unsigned char c;
    int unstress_mark;
    int length;
...
static void MatchRule(
        Translator *tr, char *word[],
        char *word_start, int group_length,
        char *rule, MatchRecord *match_out,
        int word_flags, int dict_flags) {
            ...
            if ((check_atstart == 0) || (pre_ptr[-1] == ' ')) {
                ...
                if ((option_phonemes == 2) && (match.points > 0) && ((word_flags & FLAG_NO_TRACE) == 0)) {
                    ...
                    fprintf(f_trans, "%3d\t%s [%s]\n", pts,
                            DecodeRule(group_chars, group_length, rule_start, word_flags), decoded_phonemes);
                }
            }
        }
...
/* Translate a word bounded by space characters
	   Append the result to 'phonemes' and
	   any standard prefix/suffix in 'end_phonemes' */
int TranslateRules(
        Translator *tr, char *p_start,
        char *phonemes, int ph_size,
        char *end_phonemes, int word_flags,
    ...
    if ((option_phonemes == 2) &&
        ...
        for (ix = 0; ((c = p_start[ix]) != ' ') &&
                     (c != 0) && (ix < (sizeof(wordbuf) - 1)); ix++) {
            wordbuf[ix] = c;
        }
        wordbuf[ix] = 0;
        if (word_flags & FLAG_UNPRON_TEST)
        ...
        if (IsDigit(wc) && ((tr->langopts.tone_numbers == 0) || !any_alpha)) {
            ...
            if (((ix = wc - tr->letter_bits_offset) >= 0) && (ix < 128)) {
                if (tr->groups3[ix] != NULL) {
                    MatchRule(
                            tr, &p, p_start, wc_bytes, tr->groups3[ix],
            ...
            if (!found) {
                ...
                if (tr->groups1[c] != NULL)
                    ...
                    MatchRule(tr, &p, p_start, 0, tr->groups1[0],
                              &match1, word_flags, dict_flags0);
                    ...
                    if ((match1.points == 0) &&
                        ...
                        if (tr->letter_bits_offset > 0) {
                            // not a Latin alphabet, switch to
                            // the default Latin alphabet language
                            if ((letter <= 0x241) && iswalpha2(letter)) {
                                sprintf(phonemes, "%c%s", phonSWITCH,
                                        tr->langopts.ascii_language);
                                return (0);
                            }
                        }
#ifdef deleted

                        // can't switch to a tone language, because the
                        // tone-phoneme numbers are not valid for the original language
                        if((letter >= 0x4e00) && (letter < 0xa000) &&
                        ...
                        if (letter == 0xe000 + '(') {
                            if (pre_pause < tr->langopts.param2[LOPT_BRACKET_PAUSE])
                                // a bracket, aleady spoken by AnnouncePunctuation()
                                pre_pause = tr->langopts.param2[LOPT_BRACKET_PAUSE];
                        }
                        if (IsBracket(letter)) {
                        ...
                        if ((letter >= 0xc0) && (letter < N_REMOVE_ACCENT) &&
                            ((ix = remove_accent[letter - 0xc0]) != 0)) {
                            ...
                            if ((p[-2] != ' ') || (p[n] != ' ')) {
                                ...
                                p[-1] = ix;
                                while ((p[0] = p[n]) != ' ') p++;
                                // replacement character must be no longer than original
                                while (n-- > 0) *p++ = ' ';
                                ...
                                // delete any phonemes which have been produced so far
                                phonemes[0] = 0;
                                p = p_start;
                                tr->word_vowel_count = 0;
                                tr->word_stressed_count = 0;
                ...
                if (match1.points == 0) {
                    if ((wc >= 0x300) && (wc <= 0x36f)) {
                        ...
                        if ((any_alpha > 1) || (p[wc_bytes - 1] > ' ')) {
                            // an unrecognised character in a word,
                            // abort and then spell the word
                            phonemes[0] = 0;
                            if (dict_flags != NULL)
...
#ifdef deleted
            // ?? allow $unpr while translating rules,
            // not just on initial FLAG_UNPRON_TEST
            if((match1.end_type & SUFX_UNPRON) &&
            !(word_flags & FLAG_SUFFIX_REMOVED))
                return(match1.end_type);
#endif

            if ((match1.phonemes[0] == phonSWITCH) &&
                ((word_flags & FLAG_DONT_SWITCH_TRANSLATOR) == 0)) {
                // an instruction to switch language,
                // return immediately so we can re-translate
...
void ApplySpecialAttribute2(
        Translator *tr,
        char *phonemes, int dict_flags) {
...
// transpose cyrillic alphabet (for example)
// into ascii (single byte) character codes
            ...
            if ((pairs_list = tr->frequent_pairs) != NULL) {
                ...
                for (ix = 0; c2 >= pairs_list[ix]; ix++) {
                    if (c2 == pairs_list[ix]) {
                        ...
                        // 2-character codes start after the single letter codes
                        c = ix + pairs_start;
                        p++;
                        break;
                    }
...
*/
static const char *LookupDict2(
        Translator *tr, const char *word,
        const char *word2, char *phonetic,
        unsigned int *flags, int end_flags,
        WORD_TAB *wtab) {
    char *p;
    char *next;
    int hash;
    ...
    while (*p != 0) {
        ...
        if (((p[1] & 0x7f) != wlen) ||
            ...
            // bit 6 of wlen indicates whether the word
            // has been compressed; so we need to match on this also.
            p = next;
            continue;
        }

        /* found matching entry.
         * Decode the phonetic string */
        word_end = word2;
        ...
        while (p < next) {
            ...
            if (flag >= 100) {
                ...
                // don't use the contraction
                // if any of the words are emphasized
        ...
        if ((end_flags & SUFX_P) &&
            (dictionary_flags2 & (FLAG_ONLY | FLAG_ONLY_S)))
            // $only or $onlys, don't match if a prefix has been removed
            continue;
        ...
        if ((dictionary_flags2 & FLAG_ATEND) &&
            (word_end < translator->clause_end) &&
            (lookup_symbol == 0)) {
            // only use this pronunciation
            // if it's the last word of the clause, or called from Lookup()
            continue;
        }

        if ((dictionary_flags2 & FLAG_ATSTART) &&
            !(wtab->flags & FLAG_FIRST_WORD)) {
            // only use this pronunciation
            // if it's the first word of a clause
            continue;
        }

        if ((dictionary_flags2 & FLAG_SENTENCE) &&
            !(translator->clause_terminator & CLAUSE_BIT_SENTENCE)) {
            // only if this clause is a sentence ,
        ...
        if (dictionary_flags2 & FLAG_VERB) {
            ...
            if (tr->expect_verb ||
                (tr->expect_verb_s && (end_flags & FLAG_SUFX_S))) {
                // OK, we are expecting a verb
                if ((tr->translator_name == L('e', 'n')) &&
                    (tr->prev_dict_flags[0] & FLAG_ALT6_TRANS) &&
                    (end_flags & FLAG_SUFX_S)) {
                    // lang=en, don't use verb form
                    // after 'to' if the word has 's' suffix
                    continue;
                }
            } else {
        ...
        if (option_phonemes == 2) {
            ...
            if (textmode == translator->langopts.textmode) {
                // only show this line if the word
                // translates to phonemes, not replacement text
                if ((dictionary_flags & FLAG_SKIPWORDS) && (wtab != NULL)) {
                    // matched more than one word
                    // (check for wtab prevents showing RULE_SPELLING byte
                    // when speaking individual letters)
...
*/
int LookupDictList(
        Translator *tr, char **wordptr,
        char *ph_out, unsigned int *flags,
        int end_flags, WORD_TAB *wtab) {
    int length;
    const char *found;
    const char *word1;
    const char *word2;
    ...
    while ((word2[nbytes = utf8_nbytes(word2)] == ' ') &&
        ...
        // the dots and looking for a match
    ...
    if (length > 0) {
        ...
        found = LookupDict2(
                tr, word, word2, ph_out,
                flags, end_flags, wtab);
        if (found) {
    ...
    if (flags[0] & FLAG_MAX3) {
        if (strcmp(ph_out, tr->phonemes_repeat) == 0) {
            ...
            strncpy0(tr->phonemes_repeat,
                     ph_out, sizeof(tr->phonemes_repeat));
    ...
    if (found) {
        // if textmode is the default, then
        // words which have phonemes are marked.
        ...
        if (*flags & FLAG_TEXTMODE) {
            ...
            if (end_flags & FLAG_ALLOW_TEXTMODE) {
                ...
                sprintf(&word_replacement[2], "%s ", ph_out);
...
// a word translated into phoneme codes
...
int Lookup(
    ...
    found = LookupDictList(
            tr, &word1, ph_out, flags,
            FLAG_ALLOW_TEXTMODE, NULL);
...
int RemoveEnding(
    ...
    // these lists are language specific,
    // but are only relevent if the 'e' suffix flag is used
    ...
    // look for multibyte characters to
    // increase the number of bytes to remove
    // num.of characters of the suffix
    for (len_ending = i = (end_type & 0x3f); i > 0; i--) {
        word_end--;
    ...
    // remove bytes from the end of
    // the word and replace them by spaces
    for (i = 0; (i < len_ending) &&
                (i < (int) sizeof(ending) - 1); i++) {
        ending[i] = word_end[i];
        word_end[i] = ' ';
    }
    ending[i] = 0;
    ...
    if (end_type & SUFX_E) {
        if (tr->translator_name == L('n', 'l')) {
            if (((word_end[0] & 0x80) == 0) &&
                ((word_end[-1] & 0x80) == 0) &&
                IsVowel(tr, word_end[-1]) &&
                IsLetter(tr, word_end[0], LETTERGP_C) &&
                !IsVowel(tr, word_end[-2])) {
    ...
    if ((end_type & SUFX_V) && (tr->expect_verb == 0))
        // this suffix indicates the verb pronunciation
        tr->expect_verb = 1;
    ...
    //	if(strcmp(ending,"'s")==0)
    if (ending[0] == '\'')
        // don't consider 's as an added suffix
        end_flags &= ~FLAG_SUFX;
