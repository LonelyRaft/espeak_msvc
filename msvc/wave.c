...
static int mInCallbackFinishedState = 0;
#if (USE_PORTAUDIO == 18)
...
// 				     N_WAV_BUF,pa_callback,(void *)userdata);
   ...
   mInCallbackFinishedState = 0; // v18 only
#else
      myOutputParameters.channelCount = out_channels;
      err = Pa_OpenStream(
			  &pa_stream,
...
static void select_device(const char* the_api)
{
	...
	if(selectedDeviceInfo == NULL)
	{
		...
		{
            const PaHostApiInfo *hostInfo = NULL;
		deviceInfo = Pa_GetDeviceInfo( i );
			...
			if (hostInfo && hostInfo->type == paALSA)
			{
				...
				if (defaultAlsaIndex == numDevices)
				{
                    ...
                    deviceInfo = Pa_GetDeviceInfo( defaultAlsaIndex );
...
size_t wave_write(void* theHandler, char* theMono16BitsWaveBuffer, size_t theSize)
{
	...
	if (myWrite >= aRead)
	{
        ...
        aFreeMem = myBuffer + BUFFER_LENGTH - myWrite;
...
int wave_close(void* theHandler)
{
  ...
  if( Pa_StreamActive( pa_stream ) == 0 && mInCallbackFinishedState == 0 )
    {
  ...
  //    wave_is_busy() returned 0.
  //    wave_is_busy() (which calls Pa_GetStreamActive()) will not return
  //    0 until all buffers have finished playing, so we can call
...
void add_time_in_ms(struct timespec *ts, int time_in_ms)
{
  ...
  t_ns = (uint64_t)ts->tv_nsec + 1000000 * (uint64_t)time_in_ms;
