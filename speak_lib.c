...
void espeakSleep(unsigned int _ms){
#ifdef _WIN32
    Sleep(_ms);
#endif
#ifdef __linux__
...
#ifdef USE_ASYNC
    {
        t_espeak_command* c1 = NULL;
        t_espeak_command* c2 = NULL;

        // Create the text command
        c1 = create_espeak_text(
                    text, size, position, position_type,
        ...
        *unique_identifier = c1->u.my_text.unique_identifier;
        ...
        c2 = create_espeak_terminated_msg(
...
{//===========================================
  ...
  // is there a system resource of character names per language?
    ...
    c = create_espeak_char( character, NULL);
	a_error = fifo_add_command(c);
	if (a_error != EE_OK)
	{
		delete_espeak_command(c);
...
{//=============================================================================================
  ...
  ENTER("espeak_SetParameter");
    ...
    c = create_espeak_parameter(parameter, value, relative);
...
{//================================================================
  ...
  // Set the list of punctuation which are spoken for "some".
    ...
    c = create_espeak_punctuation_list( punctlist);
	a_error = fifo_add_command(c);
	if (a_error != EE_OK)
	{
		delete_espeak_command(c);
